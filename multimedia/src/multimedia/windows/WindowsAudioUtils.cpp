...
static constexpr AudioFormat::AudioChannelPosition channelFormatMap[] =
        { AudioFormat::FrontLeft
        , AudioFormat::FrontRight
        , AudioFormat::FrontCenter
        , AudioFormat::LFE
        , AudioFormat::BackLeft
        , AudioFormat::BackRight
        , AudioFormat::FrontLeftOfCenter
        , AudioFormat::FrontRightOfCenter
        , AudioFormat::BackCenter
        , AudioFormat::SideLeft
        , AudioFormat::SideRight
        , AudioFormat::TopCenter
        , AudioFormat::TopFrontLeft
        , AudioFormat::TopFrontCenter
        , AudioFormat::TopFrontRight
        , AudioFormat::TopBackLeft
        , AudioFormat::TopBackCenter
        , AudioFormat::TopBackRight
        };
...
AudioFormat::ChannelConfig maskToChannelConfig(UINT32 mask, int count)
{
    ...
    for (auto c : channelFormatMap) {
        if (mask & 1) {
            config |= AudioFormat::channelConfig(c);
            ++set;
        }
        if (set >= count)
            break;
    ...
    return AudioFormat::ChannelConfig(config);
}

static UINT32 channelConfigToMask(AudioFormat::ChannelConfig config)
{
    UINT32 mask = 0;
    quint32 i = 0;
    for (auto c : channelFormatMap) {
        if (config & AudioFormat::channelConfig(c))
            mask |= 1 << i;
        ++i;
    }
    return mask;
...
bool formatToWaveFormatExtensible(const AudioFormat &format, WAVEFORMATEXTENSIBLE &wfx)
{
    ...
    if (format.channelCount() > 2) {
        ...
        wfx.dwChannelMask = format.channelConfig() == AudioFormat::ChannelConfigUnknown ? KSAUDIO_SPEAKER_DIRECTOUT
...
AudioFormat waveFormatExToFormat(const WAVEFORMATEX &in)
{
    ...
    if (in.wFormatTag == WAVE_FORMAT_PCM) {
        if (in.wBitsPerSample == 8)
            out.setSampleFormat(AudioFormat::UInt8);
        else if (in.wBitsPerSample == 16)
            out.setSampleFormat(AudioFormat::Int16);
        else if (in.wBitsPerSample == 32)
            out.setSampleFormat(AudioFormat::Int32);
        ...
        if (in.cbSize >= 22) {
            ...
            if (wfe.SubFormat == KSDATAFORMAT_SUBTYPE_IEEE_FLOAT)
                out.setSampleFormat(AudioFormat::Float);
...
AudioFormat mediaTypeToFormat(IMFMediaType *mediaType)
{
    AudioFormat format;
    if (!mediaType)
        return format;
    ...
    if (SUCCEEDED(mediaType->GetGUID(MF_MT_SUBTYPE, &subType))) {
        if (subType == MFAudioFormat_Float) {
            format.setSampleFormat(AudioFormat::Float);
        } else if (bitsPerSample == 8) {
            format.setSampleFormat(AudioFormat::UInt8);
        } else if (bitsPerSample == 16) {
            format.setSampleFormat(AudioFormat::Int16);
        } else if (bitsPerSample == 32){
            format.setSampleFormat(AudioFormat::Int32);
        }
    }
    return format;
}

ComPtr<IMFMediaType> formatToMediaType(WindowsMediaFoundation &wmf, const AudioFormat &format)
{
    ...
    if (format.channelConfig() != AudioFormat::ChannelConfigUnknown)
...
createAudioClient(const ComPtr<IMMDevice> &device, const AudioFormat &format,
                  ...
                  std::optional<AudioEndpointRole> role)
{
    ...
    hr = audioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, streamFlags,
                                 reference_time(periodSize).count(),
                                 reference_time(periodSize).count(),
