...
#include "WindowsAudioDevices_p.h"
...
#include <qzMultimedia/private/WindowsAudioSink_p.h>
#include <qzMultimedia/private/WindowsAudioSource_p.h>
...
QByteArray WindowsAudioDevices::s_currentDefaultOutputDeviceId;
QByteArray WindowsAudioDevices::s_currentDefaultInputDeviceId;

class CMMNotificationClient : public QComObject<IMMNotificationClient>
{
    ComPtr<IMMDeviceEnumerator> m_enumerator;
    WindowsAudioDevices *m_windowsMediaDevices;
    QMap<QString, DWORD> m_deviceState;

public:
    CMMNotificationClient(WindowsAudioDevices *windowsMediaDevices,
    ...
    HRESULT STDMETHODCALLTYPE OnDefaultDeviceChanged(EDataFlow flow, ERole role, LPCWSTR defaultDeviceId) override
    {
        if (role == ERole::eMultimedia) {
            if (defaultDeviceId) {
                QByteArray id = QString::fromWCharArray(defaultDeviceId).toUtf8();
                if (flow == EDataFlow::eRender) {
                    WindowsAudioDevices::setCurrentDefaultOutputDeviceId(id);
                    AudioDeviceMonitor::notifyOutputDeviceChanged();
                } else if (flow == EDataFlow::eCapture) {
                    WindowsAudioDevices::setCurrentDefaultInputDeviceId(id);
                }
            }
...
WindowsAudioDevices::WindowsAudioDevices()
...
QList<AudioDevice> WindowsAudioDevices::availableDevices(AudioDevice::Mode mode) const
{
    using QtMultimediaPrivate::PropertyStoreHelper;
    if (!m_deviceEnumerator)
        return {};

    const bool audioOut = mode == AudioDevice::Output;
    ...
    const auto defaultAudioDeviceID = [&, this]() -> std::optional<QString> {
        ...
        if (SUCCEEDED(m_deviceEnumerator->GetDefaultAudioEndpoint(dataFlow, ERole::eMultimedia,
                                                                  dev.GetAddressOf()))) {
            auto id = getDeviceId(dev);
            if (id) {
                QByteArray idUtf8 = id->toUtf8();
                if (audioOut)
                    s_currentDefaultOutputDeviceId = idUtf8;
                else
                    s_currentDefaultInputDeviceId = idUtf8;
            }
            return id;
        }
    ...
    for (UINT index = 0; index != numberOfDevices; ++index) {
        ...
        std::expected<PropertyStoreHelper, QString> props = PropertyStoreHelper::open(device);
        if (!props) {
        ...
        auto dev = std::make_unique<WindowsAudioDevice>(deviceId->toUtf8(), device, *friendlyName,
                                                         mode);
        dev->isDefault = deviceId == defaultAudioDeviceID;
        devices.append(AudioDevicePrivate::createQAudioDevice(std::move(dev)));
    }

    return devices;
}

QList<AudioDevice> WindowsAudioDevices::findAudioInputs() const
{
    return availableDevices(AudioDevice::Input);
}

QList<AudioDevice> WindowsAudioDevices::findAudioOutputs() const
{
    return availableDevices(AudioDevice::Output);
}

PlatformAudioSource *WindowsAudioDevices::createAudioSource(const AudioDevice &device,
                                                              const AudioFormat &fmt,
                                                              QObject *parent)
{
    return new QtWASAPI::WindowsAudioSource(device, fmt, parent);
}

PlatformAudioSink *WindowsAudioDevices::createAudioSink(const AudioDevice &device,
                                                          const AudioFormat &fmt, QObject *parent)
{
    return new QtWASAPI::WindowsAudioSink(device, fmt, parent);
}
