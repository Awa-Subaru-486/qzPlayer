...
static aaudio_format_t aaudioFormat(const AudioFormat::SampleFormat sampleFormat)
{
    switch (sampleFormat) {
    case AudioFormat::Int16:
    case AudioFormat::UInt8: // NOTE: Requires conversion
    ...
    case AudioFormat::NSampleFormats:
...
qz::Log::LogCategory qLcAAudioStream("qz.multimedia.android.aaudiostream"); // FIXME: Deprecated

StreamBuilder::StreamBuilder(AudioFormat format)
    : format{ format }
{
...
Stream::Stream(StreamBuilder &builder)
{
    ...
    if (result == AAUDIO_ERROR_INVALID_FORMAT && builder.format.sampleFormat() != AudioFormat::Float) {
        ...
        // nativeSampleFormat
        builder.format.setSampleFormat(AudioFormat::Float);
        builder.setupBuilder();
        result = AAudioStreamBuilder_openStream(builder.m_builder, &m_stream);
    }

    if (result != AAUDIO_OK) {
        qz::Log::cat_warn(qLcAAudioStream, "Opening stream failed:{}", AAudio_convertResultToText(result));
        if (isOpen())
            close();
        return;
    }

    qz::Log::cat_debug(qLcAAudioStream, "Stream opened for device{}", AAudioStream_getDeviceId(m_stream));

    // Verify parameters
    if (builder.deviceId != AAUDIO_UNSPECIFIED
        && AAudioStream_getDeviceId(m_stream) != builder.deviceId) {
        qz::Log::cat_warn(qLcAAudioStream, "Stream device not correct");
        if (isOpen())
            close();
        return;
    }
    ...
    if (builder.params.direction == AAUDIO_DIRECTION_OUTPUT
        ...
        qz::Log::cat_warn(qLcAAudioStream, "Low latency performance mode not set");
...
aaudio_result_t Stream::waitForTargetState(aaudio_stream_state_t targetState)
{
    ...
    while (result == AAUDIO_OK && currentState != targetState) {
        qz::Log::cat_debug(qLcAAudioStream, "Waiting for state change:{} -> {}", AAudio_convertStreamStateToText(currentState), AAudio_convertStreamStateToText(targetState));
...
aaudio_result_t Stream::requestWithExpectedState(Functor &&request, aaudio_stream_state_t expected)
{
    ...
    if (result != AAUDIO_OK) {
        qz::Log::cat_warn(qLcAAudioStream, "Request failed:{}", AAudio_convertResultToText(result));
        return result;
    }
