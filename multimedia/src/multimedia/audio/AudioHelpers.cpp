                      ...
                      int len) noexcept QT_MM_NONBLOCKING
{
    ...
    case AudioFormat::UInt8:
        return AudioHelperInternal::adjustSamples<quint8>(clamp(factor), src, dest, samplesCount);
    case AudioFormat::Int16:
        return AudioHelperInternal::adjustSamples<qint16>(clamp(factor), src, dest, samplesCount);
    case AudioFormat::Int32:
        return AudioHelperInternal::adjustSamples<qint32>(clamp(factor), src, dest, samplesCount);
    case AudioFormat::Float:
        return AudioHelperInternal::adjustSamples<float>(factor, src, dest, samplesCount);
    default:
                 ...
                 std::span<std::byte> destination) noexcept QT_MM_NONBLOCKING
{
    ...
    if (Q_LIKELY(volume == 1.f)) {
        ...
        std::byte zero =
                format.sampleFormat() == AudioFormat::UInt8 ? std::byte{ 0x80 } : std::byte{ 0 };

        std::fill(destination.begin(), destination.begin() + source.size(), zero);
    } else {
        AudioHelperInternal::qMultiplySamples(volume, format, source.data(), destination.data(),
                                               source.size());
    }
}
...
Q_ALWAYS_INLINE constexpr int32_t toInt32(std::span<const std::byte> source) noexcept QT_MM_NONBLOCKING
{
    switch (Format) {
...
template <NativeSampleFormat Format>
Q_ALWAYS_INLINE constexpr void storeSampleWithFormat(std::span<std::byte> destination,
...
{
    void operator()(std::span<const std::byte> source, std::span<std::byte> destination)
    {
...
void convertSampleFormatWithDestinationFormat(std::span<const std::byte> source,
                                          ...
                                          std::span<const NativeSampleFormat> supportedNativeFormats)
{
    ...
    auto resolveCandidate = [&](std::span<const NativeSampleFormat> candidates) {
    ...
    case AudioFormat::SampleFormat::NSampleFormats: {
...
NativeSampleFormat toNativeSampleFormat(AudioFormat::SampleFormat fmt)
{
    switch (fmt) {
    case AudioFormat::SampleFormat::Float:
...
AudioFormat::SampleFormat bestSampleFormat(const NativeSampleFormat fmt)
{
    switch (fmt) {
    case NativeSampleFormat::uint8_t:
        return AudioFormat::SampleFormat::UInt8;
    case NativeSampleFormat::int16_t:
        return AudioFormat::SampleFormat::Int16;
    case NativeSampleFormat::int32_t:
    case NativeSampleFormat::int24_t_3b:
    case NativeSampleFormat::int24_t_4b_low:
        return AudioFormat::SampleFormat::Int32;
    case NativeSampleFormat::float32_t:
        return AudioFormat::SampleFormat::Float;
    default:
        Q_UNREACHABLE_RETURN({});
    }
}
...
{
    constexpr float epsilon = 1.f / (1 << 22);
...
QT_MM_MAKE_STRING_RESOLVER( AudioHelperInternal::NativeSampleFormat, QtMultimediaPrivate::EnumName,
                           (AudioHelperInternal::NativeSampleFormat::uint8_t, "uint8_t")
                           (AudioHelperInternal::NativeSampleFormat::int16_t, "int16_t")
                           (AudioHelperInternal::NativeSampleFormat::int32_t, "int32_t")
                           (AudioHelperInternal::NativeSampleFormat::int24_t_3b, "int24_t_3b")
                           (AudioHelperInternal::NativeSampleFormat::int24_t_4b_low, "int24_t_4b_low")
                           (AudioHelperInternal::NativeSampleFormat::float32_t, "float32_t")
                           );

QT_MM_DEFINE_QDEBUG_ENUM(AudioHelperInternal::NativeSampleFormat);
