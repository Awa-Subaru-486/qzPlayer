...
ComPtr<IMFMediaType> MFDecoderSourceReader::setSource(IMFMediaSource *source, AudioFormat::SampleFormat sampleFormat)
{
    ...
    if (FAILED(hr)) {
        qz::Log::warn("MFDecoderSourceReader: failed to set up source reader: {}",
    ...
    pPartialType->SetGUID(MF_MT_SUBTYPE, sampleFormat == AudioFormat::Float ? MFAudioFormat_Float : MFAudioFormat_PCM);
