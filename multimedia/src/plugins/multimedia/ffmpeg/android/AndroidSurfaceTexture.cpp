...
static const char QtSurfaceTextureListenerClassName[] =
        "qz/multimedia/android/QtSurfaceTextureListener";
...
// native method for QtSurfaceTextureListener.java
static void notifyFrameAvailable(JNIEnv *, jobject, jlong id)
{
    ...
    AndroidSurfaceTexture *obj =
            reinterpret_cast<AndroidSurfaceTexture *>(g_surfaceTextures->at(idx));
    if (obj)
...
jobject AndroidSurfaceTexture::surfaceHolder()
{
    if (!m_surfaceHolder.isValid()) {
        m_surfaceHolder = QJniObject("qz/multimedia/android/QtSurfaceTextureHolder",
...
{
    m_surfaceTexture.callMethod<void>(
