...
#  endif
    ...
    /*!
        Destructor.
    */
    qreal WindowAgentBase::aspectRatio() const {
        Q_D(const WindowAgentBase);
        if (!d->context) return 0.0;
        return d->context->windowAttribute(QStringLiteral("aspect-ratio")).toReal();
    }

    void WindowAgentBase::setAspectRatio(qreal ratio) {
        Q_D(WindowAgentBase);
        if (qFuzzyCompare(aspectRatio(), ratio))
            return;
        if (!d->context) return;
        d->context->setWindowAttribute(QStringLiteral("aspect-ratio"), ratio);
        Q_EMIT aspectRatioChanged();
    }

    void WindowAgentBase::resetAspectRatio() {
        Q_D(WindowAgentBase);
        if (!d->context) return;
        d->context->setWindowAttribute(QStringLiteral("aspect-ratio"), 0.0);
        Q_EMIT aspectRatioChanged();
    }

    bool WindowAgentBase::alwaysOnTop() const {
        Q_D(const WindowAgentBase);
        if (!d->context) return false;
        return d->context->windowAttribute(QStringLiteral("always-on-top")).toBool();
    }

    void WindowAgentBase::setAlwaysOnTop(bool on) {
        Q_D(WindowAgentBase);
        if (alwaysOnTop() == on)
            return;
        if (!d->context) return;
        d->context->setWindowAttribute(QStringLiteral("always-on-top"), on);
        Q_EMIT alwaysOnTopChanged();
    }

    bool WindowAgentBase::miniMode() const {
        Q_D(const WindowAgentBase);
        if (!d->context) return false;
        return d->context->windowAttribute(QStringLiteral("mini-mode")).toBool();
    }

    void WindowAgentBase::setMiniMode(bool on) {
        Q_D(WindowAgentBase);
        if (miniMode() == on)
            return;
        if (!d->context) return;
        d->context->setWindowAttribute(QStringLiteral("auto-mini-mode"), false);
        d->context->setWindowAttribute(QStringLiteral("mini-mode"), on);
        Q_EMIT miniModeChanged();
    }

    int WindowAgentBase::miniModeThresholdWidth() const {
        Q_D(const WindowAgentBase);
        if (!d->context) return 0;
        return d->context->windowAttribute(QStringLiteral("mini-mode-threshold-width")).toInt();
    }

    void WindowAgentBase::setMiniModeThresholdWidth(int width) {
        Q_D(WindowAgentBase);
        if (miniModeThresholdWidth() == width)
            return;
        if (!d->context) return;
        d->context->setWindowAttribute(QStringLiteral("mini-mode-threshold-width"), width);
        Q_EMIT miniModeThresholdWidthChanged();
    }

    int WindowAgentBase::miniModeThresholdHeight() const {
        Q_D(const WindowAgentBase);
        if (!d->context) return 0;
        return d->context->windowAttribute(QStringLiteral("mini-mode-threshold-height")).toInt();
    }

    void WindowAgentBase::setMiniModeThresholdHeight(int height) {
        Q_D(WindowAgentBase);
        if (miniModeThresholdHeight() == height)
            return;
        if (!d->context) return;
        d->context->setWindowAttribute(QStringLiteral("mini-mode-threshold-height"), height);
        Q_EMIT miniModeThresholdHeightChanged();
    }

    qreal WindowAgentBase::miniModeAspectRatio() const {
        Q_D(const WindowAgentBase);
        if (!d->context) return 16.0 / 9.0;
        return d->context->windowAttribute(QStringLiteral("mini-mode-aspect-ratio")).toReal();
    }

    void WindowAgentBase::setMiniModeAspectRatio(qreal ratio) {
        Q_D(WindowAgentBase);
        if (qFuzzyCompare(miniModeAspectRatio(), ratio))
            return;
        if (!d->context) return;
        d->context->setWindowAttribute(QStringLiteral("mini-mode-aspect-ratio"), ratio);
        Q_EMIT miniModeAspectRatioChanged();
    }
    ...
    */
    ...
    /*!
        ...
        On Windows,
            ...
            \li \c aspect-ratio: Specify a real value to constrain the window to a specific aspect
                   ratio during resizing. Set to 0.0 to disable. The ratio is width/height (e.g.,
                   21.0/9.0 for 21:9).
