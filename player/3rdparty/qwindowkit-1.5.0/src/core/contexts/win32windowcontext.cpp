...
#  endif
    ...
    QVariant Win32WindowContext::windowAttribute(const QString &key) const {
        ...
        if (key == QStringLiteral("mini-mode-threshold-width")) {
            return m_miniModeThresholdWidth;
        }

        if (key == QStringLiteral("mini-mode-threshold-height")) {
            return m_miniModeThresholdHeight;
        }

        if (key == QStringLiteral("auto-mini-mode")) {
            return m_autoMiniMode;
        }

        if (key == QStringLiteral("mini-mode-aspect-ratio")) {
            return m_miniModeAspectRatio;
        }
    ...
    void Win32WindowContext::winIdChanged(WId winId, WId oldWinId) {
        ...
        if (!isSystemBorderEnabled()) {
            // Check if user has already set custom margins
            // MODIFICATION: Added this check to allow user to override default margins
            QVariant existingMargins = windowAttribute(QStringLiteral("extra-margins"));
            if (!existingMargins.isValid()) {
    ...
    bool Win32WindowContext::windowAttributeChanged(const QString &key, const QVariant &attribute,
        ...
        if (key == QStringLiteral("extra-margins")) {
            QMargins qMargins;
            if (attribute.type() == QVariant::Int) {
                // Handle case when user passes an integer (e.g., 0)
                // MODIFICATION: Added this check to support integer values for extra-margins
                int value = attribute.toInt();
                qMargins = QMargins(value, value, value, value);
            } else {
                // Handle case when user passes a QMargins object
                qMargins = attribute.value<QMargins>();
            }
            auto margins = qmargins2margins(qMargins);
        ...
        if (key == QStringLiteral("window-corner-preference")) {
            if (!isWin11OrGreater()) {
                return false;
            }
            int cornerPreference = attribute.toInt();
            apis.pDwmSetWindowAttribute(hwnd, _DWMWA_WINDOW_CORNER_PREFERENCE, &cornerPreference, sizeof(cornerPreference));
            return true;
        }

        if (key == QStringLiteral("aspect-ratio")) {
            return true;
        }

        if (key == QStringLiteral("always-on-top")) {
            const auto hwnd = reinterpret_cast<HWND>(m_windowId);
            if (!hwnd) return false;
            ::SetWindowPos(hwnd, attribute.toBool() ? HWND_TOPMOST : HWND_NOTOPMOST,
                           0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
            return true;
        }

        if (key == QStringLiteral("mini-mode-threshold-width")) {
            m_miniModeThresholdWidth = attribute.toInt();
            return true;
        }

        if (key == QStringLiteral("mini-mode-threshold-height")) {
            m_miniModeThresholdHeight = attribute.toInt();
            return true;
        }

        if (key == QStringLiteral("auto-mini-mode")) {
            m_autoMiniMode = attribute.toBool();
            return true;
        }

        if (key == QStringLiteral("mini-mode-aspect-ratio")) {
            m_miniModeAspectRatio = attribute.toReal();
            if (m_miniModeAspectRatio <= 0.0)
                m_miniModeAspectRatio = 16.0 / 9.0;
            return true;
        }

        if (key == QStringLiteral("mini-mode")) {
            const auto hwnd = reinterpret_cast<HWND>(m_windowId);
            if (!hwnd) return false;
            if (attribute.toBool()) {
                RECT rect{};
                ::GetWindowRect(hwnd, &rect);
                m_normalWindowRect = QRect(rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top);
                LONG style = ::GetWindowLongW(hwnd, GWL_STYLE);
                m_normalWindowStyle = style;
                style &= ~WS_MAXIMIZEBOX;
                ::SetWindowLongW(hwnd, GWL_STYLE, style);
                m_preMiniAspectRatio = windowAttribute(QStringLiteral("aspect-ratio")).toReal();
                m_isMiniMode = true;
                if (!m_autoMiniMode) {
                    qreal ratio = m_miniModeAspectRatio;
                    if (ratio <= 0.0)
                        ratio = 16.0 / 9.0;
                    setWindowAttribute(QStringLiteral("aspect-ratio"), ratio);
                    int miniW, miniH;
                    if (ratio >= 1.0) {
                        miniW = kMiniModeFixedSize;
                        miniH = qRound(static_cast<double>(miniW) / ratio);
                    } else {
                        miniH = kMiniModeFixedSize;
                        miniW = qRound(static_cast<double>(miniH) * ratio);
                    }
                    ...
                    int margin = 16;
                    int x = mi.rcWork.left + screenW - miniW - margin;
                    int y = mi.rcWork.top + screenH - miniH - margin;
                    ::SetWindowPos(hwnd, nullptr, x, y, miniW, miniH, SWP_NOZORDER | SWP_FRAMECHANGED);
                }
            } else {
                bool shouldKeepPosition = m_autoMiniMode || m_autoExitMiniMode;
                m_isMiniMode = false;
                m_autoMiniMode = false;
                m_autoExitMiniMode = false;
                LONG style = ::GetWindowLongW(hwnd, GWL_STYLE);
                style |= WS_MAXIMIZEBOX;
                ::SetWindowLongW(hwnd, GWL_STYLE, style);
                setWindowAttribute(QStringLiteral("aspect-ratio"), m_preMiniAspectRatio);
                if (shouldKeepPosition) {
                    ...
                    int restoreW = curW;
                    int restoreH = curH;
                    if (m_miniModeThresholdWidth > 0 && restoreW < m_miniModeThresholdWidth) {
                        restoreW = m_miniModeThresholdWidth + 3;
                    }
                    if (m_miniModeThresholdHeight > 0 && restoreH < m_miniModeThresholdHeight) {
                        restoreH = m_miniModeThresholdHeight + 3;
                    }
                    if (restoreW != curW || restoreH != curH) {
                        ::SetWindowPos(hwnd, nullptr,
                                       curRect.left, curRect.top,
                                       restoreW, restoreH,
                                       SWP_NOZORDER | SWP_FRAMECHANGED);
                    }
                } else if (m_normalWindowRect.isValid()) {
                    int restoreW = m_normalWindowRect.width();
                    int restoreH = m_normalWindowRect.height();
                    if (m_miniModeThresholdWidth > 0 && restoreW < m_miniModeThresholdWidth) {
                        restoreW = m_miniModeThresholdWidth + 3;
                    }
                    if (m_miniModeThresholdHeight > 0 && restoreH < m_miniModeThresholdHeight) {
                    ...
                    ::SetWindowPos(hwnd, nullptr,
                                   m_normalWindowRect.x(), m_normalWindowRect.y(),
                                   restoreW, restoreH,
                                   SWP_NOZORDER | SWP_FRAMECHANGED);
                }
            }
...
#undef SEND_MESSAGE
    ...
    bool Win32WindowContext::customWindowHandler(HWND hWnd, UINT message, WPARAM wParam,
        ...
        switch (message) {
            ...
            case WM_SIZING: {
                qreal ratio = windowAttribute(QStringLiteral("aspect-ratio")).toReal();
                if (ratio <= 0.0)
                    break;
                ...
                case WMSZ_LEFT:
                case WMSZ_RIGHT: {
                    int newH = qRound(static_cast<double>(w) / ratio);
                    rect->bottom = rect->top + newH;
                    break;
                }
                case WMSZ_TOP:
                case WMSZ_BOTTOM: {
                    int newW = qRound(static_cast<double>(h) * ratio);
            ...
            case WM_GETMINMAXINFO: {
                if (m_isMiniMode) {
                    ...
                    qreal ratio = windowAttribute(QStringLiteral("aspect-ratio")).toReal();
                    if (ratio <= 0.0)
                        ratio = m_miniModeAspectRatio;
                    int minSize = 200;
                    int maxW = qMax(640, m_miniModeThresholdWidth > 0 ? m_miniModeThresholdWidth + 1 : 640);
                    int maxH = qMax(640, m_miniModeThresholdHeight > 0 ? m_miniModeThresholdHeight + 1 : 640);
                    if (ratio >= 1.0) {
                        int minH = qRound(static_cast<double>(minSize) / ratio);
                        int maxHByRatio = qRound(static_cast<double>(maxW) / ratio);
                        mmi->ptMinTrackSize = POINT{ minSize, minH };
                        mmi->ptMaxTrackSize = POINT{ maxW, qMax(maxH, maxHByRatio) };
                    } else {
                        int minW = qRound(static_cast<double>(minSize) * ratio);
                        int maxWByRatio = qRound(static_cast<double>(maxH) * ratio);
                        mmi->ptMinTrackSize = POINT{ minW, minSize };
                        mmi->ptMaxTrackSize = POINT{ qMax(maxW, maxWByRatio), maxH };
                    }
                    *result = 0;
                    return true;
                }
            ...
            case WM_EXITSIZEMOVE: {
                if (m_isMiniMode) {
                    ...
                    int w = rect.right - rect.left;
                    int h = rect.bottom - rect.top;
                    if (w >= m_miniModeThresholdWidth && h >= m_miniModeThresholdHeight) {
                        m_autoExitMiniMode = true;
                        setWindowAttribute(QStringLiteral("mini-mode"), false);
                        Q_EMIT windowAttributeNotify(QStringLiteral("mini-mode"));
                    }
                }
                break;
            }
            ...
            case WM_SIZE: {
                if (m_miniModeThresholdWidth > 0 && m_miniModeThresholdHeight > 0) {
                    const auto hwnd = reinterpret_cast<HWND>(m_windowId);
                    RECT rect{};
                    ::GetWindowRect(hwnd, &rect);
                    int w = rect.right - rect.left;
                    ...
                    if (!m_isMiniMode) {
                        if (w < m_miniModeThresholdWidth || h < m_miniModeThresholdHeight) {
                            m_autoMiniMode = true;
                            setWindowAttribute(QStringLiteral("mini-mode"), true);
                            Q_EMIT windowAttributeNotify(QStringLiteral("mini-mode"));
                        }
                    } else if (m_autoMiniMode) {
                        if (w >= m_miniModeThresholdWidth && h >= m_miniModeThresholdHeight) {
                            m_autoExitMiniMode = true;
                            setWindowAttribute(QStringLiteral("mini-mode"), false);
                            Q_EMIT windowAttributeNotify(QStringLiteral("mini-mode"));
